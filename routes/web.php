...
Route::fallback(function (Request $request) {
    ...
    if($session){
        $shopifySession = $sessionStorage->loadSession($session->session_id);
        $appInstalled = $session ? true : false;
        $shopifyShop = Shop::all($shopifySession);
        if ($appInstalled) {
            $account = Account::where('shop', $session->shop)->first();
            $shopifyShop = count($shopifyShop) > 0 ? $shopifyShop[0] : null;
            $shopName = explode('.', $shopifyShop->domain);
            $shop_code = count($shopName) > 0 ? $shopName[0] : null;
            if($shop_code && $shop_code == 'www'){
                $shop_code = $shopName[1];
            }
            if(!$account && $shopifyShop){
                $account = Helper::storeAccount($shopifyShop, $shop_code);

                $password = Helper::randomPassword();

                $user = Helper::storeUser($account, $password);

                Helper::sendEmail($user, $password);

                Helper::storeSurvey($account);
               
                Helper::storeEmailTemplates($account); 
            }
        }
        return view('react', [
            'shop' => $shop,
